...
public class ImageUtil {
    ...
    public static void uploadAliOss(String name, InputStream input) {
        OSSClient ossClient = new OSSClient(Config.ossProtocol + Config.ossEndpoint,
                Config.ossAccessKeyId, Config.ossAccessKeySecret);
        // 上传内容到指定的存储空间（bucketName）并保存为指定的文件名称（objectName）。
        ossClient.putObject(Config.ossBucket, name, input);
        ossClient.shutdown();
    }
    ...
    public static boolean aliOssExist(String name) {
        OSSClient ossClient = new OSSClient(Config.ossProtocol + Config.ossEndpoint,
                Config.ossAccessKeyId, Config.ossAccessKeySecret);
        return ossClient.doesObjectExist(Config.ossBucket, name);
    }

    public static void aliOssDelete(String name) {
        OSSClient ossClient = new OSSClient(Config.ossProtocol + Config.ossEndpoint,
                Config.ossAccessKeyId, Config.ossAccessKeySecret);
        ossClient.deleteObject(Config.ossBucket, name);
    }

    public static String getAliOssUrl(String fileName) {
        return Config.ossUrlPrefix + fileName;
    }
