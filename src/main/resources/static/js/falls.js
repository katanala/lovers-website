    ...
    $.post("image/list", {iid: iid, per: per}, function (data) {
        ...
        for (var i = 0; i < data.length; i++) {
            html = html + "<div class='grid'><div class='imgholder'>" +
                "<img onload='imageloaded()' onerror='imageloaded()' class='thumb_photo' title='" + count + "' src='" + data[i].name + "' onclick='seeBig(this)'/>" +
                "</div></div>";
            img = img + "<img class='img' src='" + data[i].name + "'>";
            count++;
            iid = data[i].iid;
        }
